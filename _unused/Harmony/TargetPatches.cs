...
namespace Companions
{
    ...
    public static class TargetPatches
    {
        ...
        {
            /// <summary>
            /// PREFIX: During ANY gather mode (not just active harvest), skip MonsterAI's
            /// target acquisition. MonsterAI.UpdateTarget calls FindEnemy() which sets
            /// m_targetCreature. Once set, MonsterAI chases enemies instead of following
            /// the harvest waypoint/player — this is the #1 reason companions fight
            /// instead of gathering. We suppress this unless an enemy is very close
            /// and actively attacking (defensive only).
            /// </summary>
            static bool Prefix(MonsterAI __instance)
            {
                var harvest = __instance.GetComponent<HarvestController>();
                if (harvest == null || !harvest.IsInGatherMode) return true;

                // Allow UpdateTarget only for close, actively attacking enemies
                var brain = __instance.GetComponent<CompanionBrain>();
                if (brain?.Enemies != null)
                {
                    var enemies = brain.Enemies;

                    // Attacking enemy within override range — must defend
                    if (enemies.NearestAttackingEnemy != null &&
                        enemies.NearestEnemyDist < HarvestEnemyOverrideRange)
                        return true;

                    // Any enemy in melee range — can't ignore
                    if (enemies.NearestEnemy != null &&
                        enemies.NearestEnemyDist < 4f)
                        return true;
                }
